...
🔶 <b>Chain:</b> {new_data.get('chain_name')} | ⚖️️ Age: {calculate_time_difference(new_data.get('pair_created_at'))}
...
📊 <b>TS:</b> {format_large_number(new_data.get('total_supply', 0))}
👩‍👧‍👦 <b>Holders:</b> {int(format_large_number(new_data.get('holders', 0)))} | <b>Top 10:</b> {round(new_data.get('top10_percentage'), 2)}%
...
async def paginate(i):
    ...
    go_plus = await get_data_go_plus_by_address(chain=chain_name, address=ca_address)
    lp_holders = go_plus.get('holders')
    if lp_holders:
...
async def user_paginate(i, matching_users):
    address = i.get('baseToken', {}).get('address', None)
    ca_address = i.get('baseToken', {}).get('address', None)
    name = i.get('baseToken', {}).get('name', None)
    symbol = i.get('baseToken', {}).get('symbol', None)
    ...
    for user_id in matching_users:
        if await db.user_settings_crud.second_check(new_data, user_id):
            logger.info(f'Send message to {user_id}')
...
async def on_message_user(message, chain_name):
    ...
    for i in pairs:
        ...
        if users_to_notify:
            # logger.info(i)
            tasks.append(user_paginate(i, matching_users))
    logger.warning(f"Total tasks USER: {len(tasks)}")
    # logger.info(dont_first_check)
    # logger.info(len(dont_first_check))
    await asyncio.gather(*tasks)
    end = time.time()
    logger.warning(f"Total USER: {end - start_}")
...
async def on_message(message, chain_name):
    ...
    for i in pairs:
        ...
        if await token_matches_default_settings(new_data):
            ...
            # dont_first_check.append(f'{name}')
            logger.warning(f'{name} {address}, didn‘t match first check')
    logger.warning(f"Total task: {len(tasks)}")
    # logger.info(dont_first_check)
    # logger.info(len(dont_first_check))
    await asyncio.gather(*tasks)
    end = time.time()
